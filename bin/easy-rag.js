...
async function getModules() {
  ...
  return { loadConfig, saveConfig, DEFAULT_CONFIG, scanFiles, getUnembeddedChunks, embedChunks, embedTexts, saveStore, loadStore, storeExists, streamAnswer, startServer, HybridSearch };
}

const banner = `${chalk.bold.magenta('⚡ easy-rag')} ${chalk.gray('v' + pkg.version)}`;

// ── init ──────────────────────────────────────────────────────────────────────
program
  .command('init')
  .description('Create easy-rag.config.json in the current directory')
  .action(async () => {
    const { saveConfig, DEFAULT_CONFIG } = await getModules();
    const configPath = path.resolve(process.cwd(), 'easy-rag.config.json');
    if (fs.existsSync(configPath)) {
      console.log(chalk.yellow('easy-rag.config.json already exists.'));
      return;
    }
    saveConfig(DEFAULT_CONFIG);
    console.log(banner);
    console.log(chalk.green('✔ Created easy-rag.config.json'));
    console.log('\n  Edit it to set your provider, then run:');
    console.log(chalk.cyan('    npx easy-rag index'));
  });
...
program
  ...
  .action(async (opts) => {
    console.log(banner);
    const { loadConfig, scanFiles, getUnembeddedChunks, embedChunks, saveStore, HybridSearch } = await getModules();
    const config = loadConfig();
    const incremental = !opts.full;

    console.log(chalk.gray(`Provider: ${config.provider}  |  Mode: ${incremental ? 'incremental' : 'full'}  |  cwd: ${process.cwd()}`));
    console.log('');

    // Scan
    const scanSpinner = ora('Scanning files…').start();
    const { chunks, fileCount, changedFiles, skippedFiles, fileHashes } =
      await scanFiles(config, ({ file, chunkCount, reused }) => {
        scanSpinner.text = `Scanning… ${chalk.gray(file)} ${reused ? chalk.blue('[cached]') : ''} (${chunkCount} chunks)`;
      }, incremental);

    const symbolCount = chunks.filter(c => c.symbolName).length;
    scanSpinner.succeed(
      `Found ${chalk.bold(fileCount)} files → ${chalk.bold(chunks.length)} chunks, ${chalk.bold(symbolCount)} symbols` +
      (incremental ? chalk.gray(` (${changedFiles} changed, ${skippedFiles} cached)`) : '')
    );

    if (chunks.length === 0) {
      console.log(chalk.yellow('No files found. Check your include/exclude patterns.'));
      process.exit(1);
    }
    ...
    const saveSpinner = ora('Building hybrid index (BM25 + symbol graph)…').start();
    const hybrid = new HybridSearch();
    hybrid.build(chunks);
    const storePath = saveStore(chunks, { fileCount, changedFiles, skippedFiles, provider: config.provider, fileHashes }, hybrid);
    saveSpinner.succeed(`Saved to ${chalk.gray(path.relative(process.cwd(), storePath))}`);

    console.log('');
    console.log(chalk.green.bold('✔ Indexing complete!'));
    console.log('');
    console.log(chalk.cyan('    npx easy-rag ask "How does this project work?"'));
    console.log(chalk.cyan('    npx easy-rag serve'));
  });
...
program
  ...
  .action(async (question, opts) => {
    console.log(banner);
    const { loadConfig, loadStore, embedTexts, streamAnswer, HybridSearch } = await getModules();
    const config = loadConfig();
    const topK = parseInt(opts.topK) || 8;

    let store;
    try { store = loadStore(); } catch (e) { console.error(chalk.red('✘ ' + e.message)); process.exit(1); }
    ...
    async function answerQuestion(q) {
      ...
      console.log(chalk.gray(`\nRetrieved ${results.length} chunks: ${Object.entries(byType).map(([k,v]) => `${v} ${k}`).join(', ')}`));

      const uniqueFiles = [...new Set(results.map(r => r.filePath))];
      console.log(chalk.gray(`Sources: ${uniqueFiles.slice(0,5).join(', ')}${uniqueFiles.length > 5 ? ` +${uniqueFiles.length-5} more` : ''}\n`));
    ...
    if (opts.interactive || !question) {
      ...
      console.log(chalk.gray('Interactive mode — type exit or Ctrl+C to quit.\n'));

      rl.on('close', () => { console.log(chalk.gray('\nGoodbye!')); process.exit(0); });
      process.on('SIGINT', () => rl.close());

      const promptNext = () => {
        setTimeout(() => { if (!rl.closed) rl.question(chalk.cyan('You: '), handleInput); }, 80);
      };

      const handleInput = async (input) => {
        input = input.trim();
        if (!input) { promptNext(); return; }
...
program
  ...
  .action(async (opts) => {
    ...
    if (!storeExists()) {
      console.log(chalk.yellow('No index found. Run `npx easy-rag index` first.'));
      process.exit(1);
    }
...
program
  ...
  .action(async () => {
    ...
    console.log(banner);
    if (!storeExists()) { console.log(chalk.yellow('\nNo index found. Run `npx easy-rag index`.')); return; }

    const store = loadStore();
    const files = [...new Set(store.chunks.map(c => c.filePath))];
    const symbols = store.chunks.filter(c => c.symbolName);
    const byLang = {};
    for (const c of store.chunks) { byLang[c.language] = (byLang[c.language] || 0) + 1; }

    console.log('');
    console.log(`  ${chalk.bold('Files:')}     ${files.length}`);
    console.log(`  ${chalk.bold('Chunks:')}    ${store.chunks.length}`);
    console.log(`  ${chalk.bold('Symbols:')}   ${symbols.length} (functions, classes, methods)`);
    console.log(`  ${chalk.bold('Provider:')}  ${store.meta?.provider ?? '?'}`);
    console.log(`  ${chalk.bold('Mode:')}      ${store.meta?.changedFiles !== undefined ? `incremental (${store.meta.changedFiles} changed, ${store.meta.skippedFiles} cached)` : 'full'}`);
    console.log(`  ${chalk.bold('Indexed:')}   ${new Date(store.createdAt).toLocaleString()}`);
    console.log(`  ${chalk.bold('Languages:')} ${Object.entries(byLang).map(([k,v]) => `${k}(${v})`).join(' ')}`);
    console.log('');
    files.slice(0,15).forEach(f => console.log(`  ${chalk.gray('·')} ${f}`));
    if (files.length > 15) console.log(`  ${chalk.gray(`... and ${files.length-15} more`)}`);
  });
...
program
  ...
  .option('--set <key=value>', 'set a single config value (e.g. --set provider=ollama)')
  .action(async (opts) => {
    ...
    if (opts.show) {
      ...
      console.log(`  ${chalk.bold('Provider:')}         ${chalk.cyan(config.provider)}`);
      console.log('');
      console.log(`  ${chalk.bold.yellow('OpenAI')}`);
      console.log(`    API Key:         ${config.openai.apiKey ? chalk.green('set (' + config.openai.apiKey.slice(0,8) + '…)') : chalk.red('not set')}`);
    ...
    if (opts.set) {
      ...
      if (!key || value === undefined) {
        console.error(chalk.red('Usage: --set key=value  e.g. --set provider=ollama'));
        process.exit(1);
      }
      ...
      saveConfig(config);
      console.log(chalk.green(`✔ Set ${chalk.bold(resolved)} = ${chalk.cyan(value)}`));
      console.log(chalk.gray('  Run `npx easy-rag config --show` to verify.'));
      return;
    }
    ...
    const choose = async (question, options, current) => {
      ...
      options.forEach((o, i) => {
        ...
        console.log(`    ${chalk.gray(i + 1 + '.')} ${active ? chalk.cyan.bold(o.label + ' ◀ current') : o.label}`);
      });
      const answer = await ask('Enter number', String(options.findIndex(o => o.value === current) + 1));
      const idx = parseInt(answer) - 1;
      return options[idx]?.value ?? options[0].value;
    };

    console.log(chalk.bold('🔧 easy-rag configuration wizard\n'));
    ...
    if (provider === 'openai') {
      ...
      const currentKey = config.openai.apiKey || process.env.OPENAI_API_KEY || '';
      const keyHint = currentKey ? `current: ${currentKey.slice(0,8)}…` : 'sk-…';
      const apiKey = await ask('API key', keyHint);
      if (apiKey && !apiKey.startsWith('current:')) config.openai.apiKey = apiKey;

      const chatModel = await choose('Chat model', [
        { label: 'gpt-4o-mini   (fast, cheap — recommended)', value: 'gpt-4o-mini' },
      ...
      const embedModel = await choose('Embedding model', [
        { label: 'text-embedding-3-small  (fast, cheap — recommended)', value: 'text-embedding-3-small' },
        { label: 'text-embedding-3-large  (higher quality, slower)', value: 'text-embedding-3-large' },
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (availableModels.length > 0) {
            console.log(chalk.green(`\n  ✔ Connected to Ollama — found ${availableModels.length} model(s): ${availableModels.join(', ')}`));
          }
        }
      } catch {
        console.log(chalk.yellow('\n  ⚠ Could not connect to Ollama at ' + baseUrl + '. Is it running?'));
      ...
      const defaultChatModels = [
        ...
        { label: 'deepseek-coder (strong code model)', value: 'deepseek-coder' },
        { label: 'phi3           (lightweight)', value: 'phi3' },
      ];

      const chatChoices = availableModels.length > 0
        ? availableModels.map(m => ({ label: `${m}  ${chalk.green('(installed)')}`, value: m }))
        : defaultChatModels;

      const chatModel = await choose('Chat model', chatChoices, config.ollama.chatModel);
      config.ollama.chatModel = chatModel;

      const embedChoices = availableModels.length > 0
        ? availableModels.map(m => ({ label: `${m}  ${chalk.green('(installed)')}`, value: m }))
        : [
            { label: 'nomic-embed-text  (recommended for RAG)', value: 'nomic-embed-text' },
            { label: 'mxbai-embed-large (higher quality)', value: 'mxbai-embed-large' },
            { label: 'all-minilm        (lightweight)', value: 'all-minilm' },
          ];

      const embedModel = await choose('Embedding model', embedChoices, config.ollama.embeddingModel);
      config.ollama.embeddingModel = embedModel;

      // Remind about pulling models
      if (availableModels.length === 0 || !availableModels.includes(chatModel)) {
        console.log(chalk.gray(`\n  Remember to pull your models if not already done:`));
        console.log(chalk.cyan(`    ollama pull ${config.ollama.embeddingModel}`));
    ...
    saveConfig(config);
    console.log('');
    console.log(chalk.green.bold('✔ Configuration saved to easy-rag.config.json'));
    console.log('');
...
program.name('easy-rag').version(pkg.version).description('Zero-config RAG for any codebase or document folder');
program.parse();
