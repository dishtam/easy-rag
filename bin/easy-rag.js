...
const banner = `${chalk.bold.magenta('⚡ easy-rag-cli')} ${chalk.gray('v' + pkg.version)}`;
...
program
  ...
  .action(async (opts) => {
    ...
    console.log('');
    console.log(chalk.cyan('    npx easy-rag-cli ask "How does this project work?"'));
...
program
  ...
  .action(async (opts) => {
    ...
    if (!storeExists()) {
      console.log(chalk.yellow('No index found. Run `npx easy-rag-cli index` first.'));
      process.exit(1);
    }
...
program
  ...
  .action(async () => {
    ...
    console.log(banner);
    if (!storeExists()) { console.log(chalk.yellow('\nNo index found. Run `npx easy-rag-cli index`.')); return; }
...
program
  ...
  .action(async (opts) => {
    ...
    if (opts.set) {
      ...
      console.log(chalk.gray('  Run `npx easy-rag-cli config --show` to verify.'));
      return;
    }
    ...
    saveConfig(config);
    console.log('');
    console.log(chalk.green.bold('✔ Configuration saved to easy-rag-cli.config.json'));
    console.log('');
...
program.name('easy-rag-cli').version(pkg.version).description('Zero-config RAG for any codebase or document folder');
program.parse();
