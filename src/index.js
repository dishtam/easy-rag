...
export async function index(options = {}) {
  ...
  const { chunks, fileCount, changedFiles, skippedFiles, fileHashes } =
    await scanFiles(config, onProgress ? p => onProgress({ stage: 'scan', ...p }) : undefined, incremental);

  // Only embed chunks that don't already have embeddings (incremental reuse)
  const toEmbed = getUnembeddedChunks(chunks);
  if (toEmbed.length > 0) {
    await embedChunks(toEmbed, config, onProgress ? p => onProgress({ stage: 'embed', ...p }) : undefined);
  }

  const meta = { fileCount, changedFiles, skippedFiles, provider: config.provider, fileHashes };
...
export async function searchChunks(query, options = {}) {
  ...
  return hybrid.search(queryEmbedding, query, { topK }).map(({ filePath, text, rrfScore, symbolName, symbolType }) => ({
    filePath, text, score: rrfScore, symbolName, symbolType,
  }));
}
